---
type: concept
created: 2026-05-17
updated: 2026-05-17
sources: [synthesis/debriefs, sources/schaller-night-shooting]
tags: [concept, technique, recipe-behavior]
---

# Condition-dependent recipes (vs. portable looks)

Not every recipe is a filter you can apply anywhere. Some are **portable looks** that render acceptably across most conditions; others are **condition-dependent tools** that only deliver their identity under specific light + exposure, and *collapse* into generic renderings outside it. Knowing which is which prevents using a tool in conditions where it has nothing to work with.

## The split (so far)

- **Portable / versatile** — work in most lighting:
  - [[recipes/soft-color-universe]] (color default), [[recipes/brians-sepia]] (mono default, *when lit*).
- **Condition-dependent tools** — need their conditions or they fail:
  - [[recipes/loki]] & [[recipes/schaller]]: require **hard directional light + EV −2/3 underexposure**. Without both, the Shadow +4 curve has no shadows to crush and no underexposure to deepen → flat, high-key, generic. The EV −2/3 is **load-bearing, not optional seasoning**.
  - [[recipes/pastel-vibes]], [[recipes/junichiro]]: need scene *structure* (point/directional light, texture) or they go limp.
  - [[recipes/bay-area-fortia]]: needs color to amplify — rescues color-poor light, over-eggs color-rich.

## Rule

Before choosing a condition-dependent recipe, check its precondition is present. If not, either change the condition (wait for light, underexpose), or pick a portable recipe. Applying Loki/Schaller to flat shade is a category error — the recipe can't manufacture light that isn't there.

## Observations (dated, first-class)

> Self note 2026-05-17 ([[synthesis/debriefs]], frames 5155/5159): photographer flagged 5155 as "an over exposed (for it) Loki." In flat porch shade at normal exposure, both Loki (5155) and Schaller (5159) collapsed to flat/high-key — neither look survived without hard light + EV −2/3. Confirms the split: these two are tools, not filters.

> External corroboration 2026-05-17 ([[sources/schaller-night-shooting]], [[synthesis/schaller-technique]]): the underexpose-for-highlights precondition behind [[recipes/schaller|Schaller Dark]] is **Schaller's own deliberate method**, not just our inference — he shoots EV ≈ −2 because *"most of the dynamic range comes from boosting out of the shadows rather than cutting back on highlight."* Strengthens "EV underexposure is load-bearing." Magnitude caveat: his −2 assumes RAW + post shadow-boost; the JPEG recipe's −2/3 is the in-camera analog (can't lift as far).

## Open / to deepen

- Where exactly is Junichiro/Pastel Vibes on the spectrum — how much structure is "enough"? (untested boundary)
- Does any amount of EV −2/3 alone (without hard light) partially rescue Loki/Schaller in shade?
